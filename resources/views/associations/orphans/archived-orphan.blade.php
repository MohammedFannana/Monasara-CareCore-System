<x-main-layout title="1000 أمل لأبناء الشهداء">
    <div class="header fw-bold" style="color:var(--primary-color);font-size:18px;border-radius:6px;padding:10px;background:linear-gradient(to left,#c6fdda,#edfaf1)">
        ...
        <p class="fw-normal mt-2" style="font-size:15px;color:var(--text-color)">
            يعرض هذا القسم الأيتام المؤرشفين التابعين لجمعيتك.
        </p>
    </div>

    <section class="family-information mt-5">
        <div class="rounded">
            <form action="{{ route('association.orphan.archived') }}" method="GET" class="search custom-sm-style w-100">
            ...
            <div class="table-responsive">
                <table class="border-0 w-100 text-center" style="border-collapse:collapse;">
                    <thead>
                        <tr><th>#</th><th>اسم اليتيم</th><th>الجنس</th><th>الدولة</th><th>الإجراءات</th></tr>
                    </thead>
                    <tbody>
                        @forelse ($orphans as $orphan)
                            <tr>
                                ...
                                <td><a href="{{ route('association.orphan.view1', $orphan) }}" class="text-decoration-none">عرض التفاصيل</a></td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="text-center fs-5" style="color:var(--primary-color)">لا يوجد أيتام مؤرشفون</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
